...
// Listen for changes on 'scp_availability' table
const scpAvailabilityChannel = realtime.channel("public:scp_availability");
scpAvailabilityChannel.on(
  "postgres_changes",
  { event: "UPDATE", schema: "public", table: "scp_availability" },
  (payload) => {
    triggerFunction(payload);
  }
);
scpAvailabilityChannel.subscribe();
